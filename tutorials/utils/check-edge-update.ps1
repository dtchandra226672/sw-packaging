...
$WebResult = Invoke-WebRequest -Uri $Link | Select-Object -Expand Content 
$txtFileName = 'C:\Users\Darryl Chandra\tutorials\utils\edge-latest-ver.txt'

$ParseWebsite = '<h2 id="version[^"]+">Version ([\d\.]+)'
$ParseVersion = '(\d+\.\d)\.(\d+)\.(\d+)'

$Result = [regex]::Matches($WebResult, $ParseWebsite).Groups[0].Value

if ($Result -match $ParseVersion) {
    $Version = $Matches[0]
...
if (-Not (Test-Path "$txtFileName")) {
    ...
    Write-Host 'New File Successfully Created! Now Proceeding...' -ForegroundColor Blue
